---
summary: "Tham khảo CLI OpenClaw cho các lệnh, subcommand và tùy chọn của `openclaw`"
read_when:
...
Trang này mô tả hành vi hiện tại của CLI. Nếu lệnh thay đổi, cập nhật tài liệu này.
...
- Hyperlink OSC-8 hiển thị dưới dạng liên kết có thể nhấp trong terminal hỗ trợ; nếu không, sẽ hiển thị URL đơn giản.
- `--json` (và `--plain` nếu hỗ trợ) tắt định dạng để có đầu ra sạch.
- `--no-color` tắt định dạng ANSI; `NO_COLOR=1` cũng được tôn trọng.
- Lệnh chạy lâu hiển thị chỉ báo tiến trình (OSC 9;4 nếu hỗ trợ).
...
Lưu ý: plugin có thể thêm lệnh cấp cao nhất (ví dụ `openclaw voicecall`).

## Bảo mật

- `openclaw security audit` — kiểm tra cấu hình + trạng thái local để tìm lỗi bảo mật phổ biến.
- `openclaw security audit --deep` — nỗ lực tốt nhất để kiểm tra Gateway trực tiếp.
...
- `openclaw secrets audit` — quét các dư lượng plaintext, tham chiếu chưa được giải quyết và sự trôi dạt ưu tiên (`--allow-exec` để thực thi các nhà cung cấp exec trong quá trình kiểm tra).
- `openclaw secrets configure` — trợ giúp tương tác cho thiết lập nhà cung cấp + ánh xạ SecretRef + kiểm tra trước/áp dụng (`--allow-exec` để thực thi các nhà cung cấp exec trong các luồng áp dụng và kiểm tra trước có chứa exec).
- `openclaw secrets apply --from <plan.json>` — áp dụng một kế hoạch đã tạo trước đó (`--dry-run` được hỗ trợ; sử dụng `--allow-exec` để cho phép các nhà cung cấp exec trong kế hoạch ghi có chứa exec và chạy thử).

## Plugins

Quản lý các phần mở rộng và cấu hình của chúng:

- `openclaw plugins list` — khám phá plugin (sử dụng `--json` để có đầu ra máy).
...
- `openclaw memory index` — lập chỉ mục lại các file memory.
- `openclaw memory search "<query>"` (hoặc `--query "<query>"`) — tìm kiếm ngữ nghĩa qua memory.

## Chat slash commands

Tin nhắn chat hỗ trợ lệnh `/...` (văn bản và gốc). Xem [/tools/slash-commands](/tools/slash-commands).

Điểm nổi bật:

- `/status` để chẩn đoán nhanh.
- `/config` để thay đổi cấu hình được lưu trữ.
- `/debug` để ghi đè cấu hình chỉ runtime (memory, không phải disk; yêu cầu `commands.debug: true`).
...
- `--workspace <dir>`: đường dẫn workspace agent (mặc định `~/.openclaw/workspace`).
...
Onboarding tương tác cho gateway, workspace và skills.
...
- `--secret-input-mode <plaintext|ref>` (mặc định `plaintext`; sử dụng `ref` để lưu trữ tham chiếu môi trường nhà cung cấp mặc định thay vì khóa plaintext)
...
- `--custom-api-key <key>` (không tương tác; tùy chọn; sử dụng với `--auth-choice custom-api-key`; dự phòng `CUSTOM_API_KEY` khi không có)
- `--custom-provider-id <id>` (không tương tác; tùy chọn id nhà cung cấp tùy chỉnh)
- `--custom-compatibility <openai|anthropic>` (không tương tác; tùy chọn; mặc định `openai`)
- `--gateway-port <port>`
- `--gateway-bind <loopback|lan|tailnet|auto|custom>`
- `--gateway-auth <token|password>`
- `--gateway-token <token>`
- `--gateway-token-ref-env <name>` (không tương tác; lưu `gateway.auth.token` dưới dạng SecretRef môi trường; yêu cầu biến môi trường đó được đặt; không thể kết hợp với `--gateway-token`)
...
Trợ giúp cấu hình không tương tác (get/set/unset/file/validate). Chạy `openclaw config` không có
subcommand sẽ khởi chạy trình hướng dẫn.

Subcommands:

- `config get <path>`: in giá trị cấu hình (đường dẫn dot/bracket).
- `config set`: hỗ trợ bốn chế độ gán:
  - chế độ giá trị: `config set <path> <value>` (phân tích JSON5-hoặc-chuỗi)
  ...
  - chế độ batch: `config set --batch-json '<json>'` hoặc `config set --batch-file <path>`
- `config set --dry-run`: xác thực gán mà không ghi `openclaw.json` (kiểm tra SecretRef exec bị bỏ qua theo mặc định).
- `config set --allow-exec --dry-run`: chọn tham gia kiểm tra SecretRef exec dry-run (có thể thực thi lệnh nhà cung cấp).
- `config set --dry-run --json`: phát ra đầu ra dry-run có thể đọc được bằng máy (kiểm tra + tín hiệu hoàn chỉnh, hoạt động, tham chiếu được kiểm tra/bỏ qua, lỗi).
- `config set --strict-json`: yêu cầu phân tích JSON5 cho đầu vào đường dẫn/giá trị. `--json` vẫn là bí danh cũ cho phân tích nghiêm ngặt ngoài chế độ đầu ra dry-run.
- `config unset <path>`: xóa một giá trị.
- `config file`: in đường dẫn file cấu hình đang hoạt động.
- `config validate`: xác thực cấu hình hiện tại so với schema mà không khởi động gateway.
...
- `--non-interactive`: bỏ qua nhắc nhở; chỉ áp dụng di chuyển an toàn.
- `--deep`: quét dịch vụ hệ thống để tìm cài đặt gateway bổ sung.
...
- `channels status`: kiểm tra khả năng truy cập gateway và sức khỏe kênh (`--probe` chạy kiểm tra bổ sung; sử dụng `openclaw health` hoặc `openclaw status --deep` để kiểm tra sức khỏe gateway).
- Mẹo: `channels status` in cảnh báo với các đề xuất sửa lỗi khi phát hiện cấu hình sai phổ biến (sau đó chỉ bạn đến `openclaw doctor`).
- `channels logs`: hiển thị nhật ký kênh gần đây từ file nhật ký gateway.
- `channels add`: thiết lập kiểu wizard khi không có cờ nào được truyền; cờ chuyển sang chế độ không tương tác.
  - Khi thêm tài khoản không mặc định vào kênh vẫn sử dụng cấu hình cấp cao nhất cho một tài khoản, OpenClaw di chuyển các giá trị có phạm vi tài khoản vào `channels.<channel>.accounts.default` trước khi ghi tài khoản mới.
  - `channels add` không tương tác không tự động tạo/nâng cấp ràng buộc; ràng buộc chỉ kênh tiếp tục khớp với tài khoản mặc định.
- `channels remove`: tắt theo mặc định; truyền `--delete` để xóa các mục cấu hình mà không nhắc nhở.
...
- `--no-usage`: bỏ qua ảnh chụp nhanh sử dụng/định mức nhà cung cấp mô hình (chỉ OAuth/API-backed).
...
Phê duyệt yêu cầu ghép đôi DM qua các kênh.
...
Quản lý mục ghép đôi thiết bị gateway và token thiết bị theo vai trò.
...
Nhắn tin outbound hợp nhất + hành động kênh.
...
- `--model <id>`
- `--agent-dir <dir>`
- `--bind <channel[:accountId]>` (lặp lại được)
- `--non-interactive`
- `--json`

Thông số ràng buộc sử dụng `channel[:accountId]`. Khi `accountId` bị bỏ qua, OpenClaw có thể giải quyết phạm vi tài khoản qua mặc định kênh/plugin hooks; nếu không, đó là ràng buộc kênh mà không có phạm vi tài khoản rõ ràng.

#### `agents bindings`

Liệt kê ràng buộc định tuyến.
...
Xóa một agent và loại bỏ workspace + trạng thái của nó.
...
Chạy cầu nối ACP kết nối IDEs với Gateway.

Xem [`acp`](/cli/acp) để biết đầy đủ tùy chọn và ví dụ.

### `status`

Hiển thị sức khỏe phiên liên kết và người nhận gần đây.
...
- `--deep` (kiểm tra kênh)
- `--usage` (hiển thị sử dụng/định mức nhà cung cấp mô hình)
- `--timeout <ms>`
- `--verbose`
...
- Tổng quan bao gồm trạng thái dịch vụ host Gateway + node khi có sẵn.

### Theo dõi sử dụng

OpenClaw có thể hiển thị sử dụng/định mức nhà cung cấp khi có OAuth/API creds.
...
- Dữ liệu đến trực tiếp từ điểm cuối sử dụng nhà cung cấp (không có ước tính).
- Nhà cung cấp: Anthropic, GitHub Copilot, OpenAI Codex OAuth, cộng với Gemini CLI qua plugin `google` đi kèm và Antigravity khi được cấu hình.
- Nếu không có thông tin xác thực phù hợp, sử dụng bị ẩn.
...
Đặt lại cấu hình/trạng thái local (giữ CLI đã cài đặt).
...
Gỡ cài đặt dịch vụ gateway + dữ liệu local (CLI vẫn còn).
...
- `gateway status` kiểm tra Gateway RPC theo mặc định bằng cách sử dụng cổng/cấu hình đã giải quyết của dịch vụ (ghi đè bằng `--url/--token/--password`).
- `gateway status` hỗ trợ `--no-probe`, `--deep`, `--require-rpc`, và `--json` cho scripting.
- `gateway status` cũng hiển thị các dịch vụ gateway cũ hoặc bổ sung khi có thể phát hiện chúng (`--deep` thêm quét cấp hệ thống). Dịch vụ OpenClaw có tên profile được coi là hạng nhất và không bị gắn cờ là "bổ sung".
- `gateway status` in đường dẫn cấu hình mà CLI sử dụng so với cấu hình mà dịch vụ có thể sử dụng (môi trường dịch vụ), cộng với URL mục tiêu kiểm tra đã giải quyết.
- Nếu các SecretRefs xác thực gateway chưa được giải quyết trong đường dẫn lệnh hiện tại, `gateway status --json` báo cáo `rpc.authWarning` chỉ khi kết nối/kiểm tra xác thực thất bại (cảnh báo bị ẩn khi kiểm tra thành công).
- Trên cài đặt Linux systemd, kiểm tra trôi dạt token trạng thái bao gồm cả nguồn `Environment=` và `EnvironmentFile=` của đơn vị.
...
Theo dõi nhật ký file Gateway qua RPC.

Ghi chú:

- Phiên TTY hiển thị chế độ xem có màu sắc, có cấu trúc; không phải TTY sẽ quay lại văn bản đơn giản.
- `--json` phát ra JSON phân cách dòng (một sự kiện nhật ký mỗi dòng).
...
Trợ giúp CLI Gateway (sử dụng `--url`, `--token`, `--password`, `--timeout`, `--expect-final` cho subcommand RPC).
...
sử dụng đăng ký ngoài Claude Code trong quá khứ; xác minh điều khoản Anthropic hiện tại trước khi dựa vào setup-token trong sản xuất.

### `models` (gốc)

`openclaw models` là bí danh cho `models status`.
...
Quản lý công việc đã lên lịch (Gateway RPC). Xem [/automation/cron-jobs](/automation/cron-jobs).
...
`node` chạy một **node host không đầu** hoặc quản lý nó như một dịch vụ nền. Xem
...
- `node` giải quyết xác thực gateway từ env/config (không có cờ `--token`/`--password`): `OPENCLAW_GATEWAY_TOKEN` / `OPENCLAW_GATEWAY_PASSWORD`, sau đó `gateway.auth.*`. Trong chế độ local, node host cố ý bỏ qua `gateway.remote.*`; trong `gateway.mode=remote`, `gateway.remote.*` tham gia theo quy tắc ưu tiên từ xa.
- Các biến môi trường `CLAWDBOT_GATEWAY_*` cũ cố ý bị bỏ qua để giải quyết xác thực node-host.
...
- `nodes run --node <id|name|ip> [--cwd <path>] [--env KEY=VAL] [--command-timeout <ms>] [--needs-screen-recording] [--invoke-timeout <ms>] <command...>` (mac node hoặc node host không đầu)
...
- `--thinking <level>`
- `--message <text>`
- `--timeout-ms <ms>` (mặc định `agents.defaults.timeoutSeconds`)
- `--history-limit <n>`\n