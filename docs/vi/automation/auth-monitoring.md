...
read_when:
  - Thiết lập giám sát hoặc cảnh báo thời hạn xác thực
...
## Script tùy chọn (cho vận hành / quy trình điện thoại)

Các script này nằm trong thư mục `scripts/` và là **tùy chọn**. Chúng giả định có quyền truy cập SSH vào máy chủ gateway và được tối ưu hóa cho systemd + Termux.

- `scripts/claude-auth-status.sh` hiện sử dụng `openclaw models status --json` làm nguồn thông tin chính (sẽ đọc trực tiếp từ file nếu CLI không khả dụng), vì vậy hãy đảm bảo `openclaw` có trong `PATH` cho các bộ đếm thời gian.
...
- `scripts/mobile-reauth.sh`: quy trình xác thực lại có hướng dẫn qua SSH.
